...
interface CastRulesSectionProps {
  ...
  onAddSubChar: (sub?: Partial<SubCharacter>) => void;
  onUpdateSubChar: (index: number, sub: Partial<SubCharacter>) => void;
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-foreground flex items-center gap-2">
              ...
              <span className="text-[10px] px-2.5 py-0.5 rounded-full font-bold bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-500/20 flex items-center gap-1">
                ...
                <span>ไม่จำกัดจำนวน (Unlimited)</span>
              </span>
            </h2>
        ...
        <button
          ...
          className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-xs font-bold shadow-xs transition-all flex items-center gap-1.5 cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
            ...
            <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">• ไม่หักโควตาตัวละครหลัก</span>
          </h3>
        </div>
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className="w-5 h-5 rounded-md bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold flex items-center justify-center flex-shrink-0">
                      {idx + 1}
                    </span>
                    <input
                      type="text"
                      ...
                      placeholder="ชื่อตัวละครเสริม เช่น สปาร์กี้ (นกฮูกจักรกล)..."
                  ...
                  <button
                    ...
                    className="w-6 h-6 rounded-md hover:bg-rose-500/20 text-muted-foreground hover:text-rose-600 dark:hover:text-rose-400 flex items-center justify-center transition-all cursor-pointer"
