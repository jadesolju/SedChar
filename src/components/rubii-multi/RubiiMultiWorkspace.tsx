...
const TABS = [
  { id: 'world', label: '1. World Setting', desc: 'กฎของโลก, ฉาก, ยุคสมัย', icon: Globe, color: 'text-purple-400' },
  { id: 'lore', label: '2. Lore & Timeline', desc: 'ไทม์ไลน์, ภูมิหลัง, ความลับ', icon: BookOpen, color: 'text-amber-400' },
  { id: 'routes', label: '3. Routes & Branches', desc: 'เส้นทางเรื่อง, เงื่อนไข', icon: GitFork, color: 'text-blue-400' },
  { id: 'mainChars', label: '4. Main Characters', desc: 'ตัวละครหลัก (10 ตัวใน Free)', icon: Users, color: 'text-rose-400' },
  { id: 'castRules', label: '5. Cast & Sub-Chars', desc: 'ตัวละครเสริมไม่จำกัด + กฎฉากรวม', icon: Sparkles, color: 'text-emerald-400' },
  { id: 'preview', label: '6. Rubii Draft Export', desc: 'สรุปรวมร่างโปรเจกต์', icon: FileText, color: 'text-pink-400' },
];
...
export function RubiiMultiWorkspace() {
  ...
  const handleLoadSample = () => {
    loadSampleProject();
    showToast('โหลดตัวอย่างโปรเจกต์ "มหานครกรงทอง (ซ้อเหมย & วาเลน)" เรียบร้อย!');
  };
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background text-foreground font-sans">
      ...
      <header className="flex-shrink-0 h-14 border-b border-border bg-card/80 backdrop-blur-md px-3 sm:px-5 flex items-center justify-between z-30">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.2 rounded bg-rose-500/15 text-rose-300 border border-rose-500/30">
                  Preview
                </span>
                <span className="hidden sm:inline-block text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.2 rounded bg-purple-500/15 text-purple-300 border border-purple-500/30">
                  Rubii Only
                </span>
              </div>
            </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl border border-border bg-card hover:bg-muted text-xs font-semibold text-foreground transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
            title="โหลดตัวอย่างโปรเจกต์หลายตัวละคร"
          >
          ...
          <button
            type="button"
            onClick={handleReset}
            className="w-8 h-8 rounded-xl border border-border bg-card hover:bg-rose-500/20 text-muted-foreground hover:text-rose-400 transition-all flex items-center justify-center cursor-pointer shadow-xs"
      ...
      <div className="flex-1 overflow-hidden flex relative">
        ...
        <aside className="hidden md:flex flex-col w-64 lg:w-72 border-r border-border bg-card/40 p-3 space-y-1.5 flex-shrink-0 overflow-y-auto custom-scrollbar">
          <div className="px-2 py-1.5 text-[11px] font-bold uppercase tracking-wider text-muted-foreground flex items-center justify-between">
            ...
            <span className="text-[10px] text-rose-400 font-bold font-mono">{mainCharCount}/{maxMainChars} ตัวหลัก</span>
          </div>
            ...
            return (
              ...
              >
                <div className={"w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 " + (isActive ? "bg-rose-500/15 border border-rose-500/30" : "bg-muted/60")}>
                  <Icon className={"w-4 h-4 " + (isActive ? "text-rose-400" : tab.color)} />
                </div>
          ...
          <div className="mt-auto pt-4 border-t border-border px-2">
            <div className="p-3 rounded-xl bg-muted/40 border border-border text-[11px] text-muted-foreground space-y-1">
              <div className="font-bold text-foreground flex items-center gap-1.5">
                <span>🛡️ Rubii Multi-Char Engine</span>
              </div>
        ...
        {isMobileDrawerOpen && (
          <div className="fixed inset-0 z-40 md:hidden bg-black/80 backdrop-blur-xs animate-in fade-in flex flex-col">
              ...
              {TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={"w-full p-3 rounded-2xl border text-left flex items-center gap-3 cursor-pointer " + (isActive ? "bg-rose-500/15 border-rose-500/50" : "bg-card border-border")}
                  >
      ...
      {/* Toast Notification */}
      {toastMsg && (
        <div className="fixed bottom-4 right-4 z-50 px-4 py-2.5 rounded-xl bg-neutral-900 border border-rose-500/40 text-white text-xs font-bold shadow-2xl flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2">
